...
public class CreatureHandler
{
	...
	*/
	public void updateCreatures() {
		...
		//int _creaturesLength = _creatures.size();
		...
		for(int i = 0; i < _creatures.size(); i++) {
			...
			// Skip dead creatures
			if(!(_creatures.get(i).isAlive()))
				continue;

			if(_creatures.get(i) instanceof Aware) {
				Creature above = getTarget(_creatures.get(i), 0, -1);
				Creature below = getTarget(_creatures.get(i), 0, 1);
				Creature left = getTarget(_creatures.get(i), -1, 0);
				Creature right = getTarget(_creatures.get(i), 1, 0);

				Aware a = (Aware)_creatures.get(i);
				a.senseNeighbors(above, below, left, right);
			}

			if(_creatures.get(i) instanceof Movable) {
				handleMove(_creatures.get(i));
			}

			if(_creatures.get(i) instanceof Aggressor) {
				Creature target = getTarget(_creatures.get(i), 0, 0);
				Aggressor a = (Aggressor) _creatures.get(i);
				a.attack(target);
			}
			...
			if (_creatures.get(i) instanceof Spawner) {
				Wolf spawner = (Wolf) _creatures.get(i);
				if (spawner._spawn) {
					 _creatures.add(spawner.spawnNewCreature());
				}
			}
